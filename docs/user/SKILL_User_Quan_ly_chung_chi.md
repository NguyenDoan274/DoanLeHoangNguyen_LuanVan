# SKILL.md — Use-case: Quản lý chứng chỉ

## 1. Mục tiêu use-case

Use-case **Quản lý chứng chỉ** cho phép học viên xem, tải và xác minh chứng chỉ khi hệ thống có bật chức năng cấp chứng chỉ.
...
> Ghi chú: Nếu MVP hiện tại đã bỏ bảng certificates, có thể dùng file này như tài liệu mở rộng phase sau.
...
Use-case **Quản lý chứng chỉ** bao gồm:
...
Tải file chứng chỉ
Xác minh chứng chỉ bằng mã
Hiển thị thông tin khóa học và người học trên chứng chỉ
```

Không bao gồm:

```txt
Thiết kế mẫu certificate nâng cao
Thanh toán
Duyệt khóa học
```
...
### 4.1. Tạo chứng chỉ khi hoàn thành
...
| Hậu điều kiện | Hệ thống hoàn tất luồng **Tạo chứng chỉ khi hoàn thành**, dữ liệu được cập nhật trong bảng liên quan và frontend hiển thị trạng thái mới. |
...
|---|---|
| Tiền điều kiện | Actor đã đăng nhập nếu chức năng yêu cầu xác thực; tài khoản phải ở trạng thái `ACTIVE`; dữ liệu liên quan phải tồn tại trong database. |
| Hậu điều kiện | Hệ thống hoàn tất luồng **Xem chứng chỉ**, dữ liệu được cập nhật trong bảng liên quan và frontend hiển thị trạng thái mới. |

### 4.3. Xác minh chứng chỉ

| Mục | Nội dung |
|---|---|
| Tiền điều kiện | Actor đã đăng nhập nếu chức năng yêu cầu xác thực; tài khoản phải ở trạng thái `ACTIVE`; dữ liệu liên quan phải tồn tại trong database. |
| Hậu điều kiện | Hệ thống hoàn tất luồng **Xác minh chứng chỉ**, dữ liệu được cập nhật trong bảng liên quan và frontend hiển thị trạng thái mới. |

---

## 5. Database liên quan

Use-case này chủ yếu sử dụng các bảng: `users`, `courses`, `enrollments`, `lesson_progress`, `quizzes`, `quiz_attempts`, `certificates`.
...
Table certificates {
  ...
  certificate_code varchar [not null, unique]
...
- `users`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `courses`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `enrollments`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `lesson_progress`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `quizzes`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `quiz_attempts`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
- `certificates`: phục vụ lưu trữ/truy vấn dữ liệu cho use-case **Quản lý chứng chỉ**.
...
NestJS Backend
   |
   | CertificatesModule / Guards / Services
   v
Prisma ORM
   |
   v
...
|---|---|
| `certificates.controller.ts` | Nhận request, đọc user từ JWT, gọi service và trả response |
| `certificates.service.ts` | Xử lý nghiệp vụ chính của use-case **Quản lý chứng chỉ** |
...
### 7.1. Tạo chứng chỉ
```http
POST /courses/:courseId/certificates/generate
...
```json
{
  "message": "Xem chứng chỉ của tôi thành công",
  "data": {
    "id": "uuid"
  }
}
...
```json
{
  "message": "Xem chứng chỉ thành công",
  "data": {
    "id": "uuid"
  }
}
...
```json
{
  "message": "Tải certificate PDF thành công",
  "data": {
    "id": "uuid"
  }
}
...
### 8.1. Data flow — Tạo chứng chỉ khi hoàn thành

```txt
User mở màn hình /my-certificates
→ Next.js kiểm tra trạng thái đăng nhập
→ Next.js gửi POST /courses/:courseId/certificates/generate
→ NestJS JwtAuthGuard xác thực access token
→ RolesGuard kiểm tra quyền phù hợp với use-case
→ CertificatesController nhận request
→ CertificatesService kiểm tra dữ liệu đầu vào
→ Service kiểm tra quan hệ dữ liệu và quyền sở hữu nếu có
→ Service gọi Prisma để thao tác bảng chính: certificates
...
### 8.2. Data flow — Xem chứng chỉ

```txt
User mở màn hình /my-certificates
→ Next.js kiểm tra trạng thái đăng nhập
→ Next.js gửi GET /my-certificates
→ NestJS JwtAuthGuard xác thực access token
→ RolesGuard kiểm tra quyền phù hợp với use-case
→ CertificatesController nhận request
→ CertificatesService kiểm tra dữ liệu đầu vào
...
### 8.3. Data flow — Xác minh chứng chỉ

```txt
User mở màn hình /my-certificates
→ Next.js kiểm tra trạng thái đăng nhập
→ Next.js gửi GET /certificates/:code
→ NestJS JwtAuthGuard xác thực access token
→ RolesGuard kiểm tra quyền phù hợp với use-case
→ CertificatesController nhận request
→ CertificatesService kiểm tra dữ liệu đầu vào
...
### 9.1. Sequence — Tạo chứng chỉ khi hoàn thành
...
participant "CertificatesController" as Controller
participant "CertificatesService" as Service
participant "PrismaService" as Prisma
database "PostgreSQL" as DB

Actor -> FE: Thực hiện thao tác: Tạo chứng chỉ khi hoàn thành
FE -> Guard: POST /courses/:courseId/certificates/generate + JWT
...
alt Token không hợp lệ hoặc hết hạn
    ...
    Controller -> Service: Xử lý nghiệp vụ Tạo chứng chỉ khi hoàn thành
...
Actor -> FE: Thực hiện thao tác: Xem chứng chỉ
FE -> Guard: GET /my-certificates + JWT
Guard -> Guard: Verify JWT và kiểm tra role
...
alt Token không hợp lệ hoặc hết hạn
    ...
    Guard -> Controller: Cho phép request
    Controller -> Service: Xử lý nghiệp vụ Xem chứng chỉ
    Service -> Prisma: Truy vấn/cập nhật dữ liệu liên quan
    Prisma -> DB: SELECT/INSERT/UPDATE
    DB --> Prisma: Kết quả
    Prisma --> Service: Dữ liệu đã xử lý
...
### 9.3. Sequence — Xác minh chứng chỉ
...
Actor -> FE: Thực hiện thao tác: Xác minh chứng chỉ
FE -> Guard: GET /certificates/:code + JWT
Guard -> Guard: Verify JWT và kiểm tra role
...
alt Token không hợp lệ hoặc hết hạn
    ...
    Guard -> Controller: Cho phép request
    Controller -> Service: Xử lý nghiệp vụ Xác minh chứng chỉ
    Service -> Prisma: Truy vấn/cập nhật dữ liệu liên quan
    Prisma -> DB: SELECT/INSERT/UPDATE
    DB --> Prisma: Kết quả
    Prisma --> Service: Dữ liệu đã xử lý
...
### 10.1. Activity flow — Tạo chứng chỉ khi hoàn thành

```txt
Bắt đầu
→ Actor chọn chức năng: Tạo chứng chỉ khi hoàn thành
...
### 10.2. Activity flow — Xem chứng chỉ

```txt
Bắt đầu
→ Actor chọn chức năng: Xem chứng chỉ
→ Hệ thống kiểm tra đăng nhập
   ├── Chưa đăng nhập → chuyển đến /login
   └── Đã đăng nhập
       → Kiểm tra role
...
### 10.3. Activity flow — Xác minh chứng chỉ

```txt
Bắt đầu
→ Actor chọn chức năng: Xác minh chứng chỉ
→ Hệ thống kiểm tra đăng nhập
   ├── Chưa đăng nhập → chuyển đến /login
   └── Đã đăng nhập
       → Kiểm tra role
...
|---|---|---|
| `POST /courses/:courseId/certificates/generate` | STUDENT hoặc user đã đăng nhập | Tạo chứng chỉ |
| `GET /my-certificates` | STUDENT hoặc user đã đăng nhập | Xem chứng chỉ của tôi |
| `GET /certificates/:code` | STUDENT hoặc user đã đăng nhập | Xem chứng chỉ |
| `GET /certificates/verify/:code` | STUDENT hoặc user đã đăng nhập | Xác minh chứng chỉ |
| `GET /certificates/:code/download` | STUDENT hoặc user đã đăng nhập | Tải certificate PDF |
...
- Kiểm tra điều kiện nhận chứng chỉ: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
- Sinh mã chứng chỉ: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
- Xem danh sách chứng chỉ: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
- Tải file chứng chỉ: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
- Xác minh chứng chỉ bằng mã: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
- Hiển thị thông tin khóa học và người học trên chứng chỉ: kiểm tra dữ liệu bắt buộc, quyền truy cập, trạng thái bản ghi và quan hệ khóa ngoại trước khi xử lý.
...
### 15.1. Flow — Tạo chứng chỉ khi hoàn thành

```txt
/my-certificates
→ Người dùng chọn chức năng: Tạo chứng chỉ khi hoàn thành
...
### 15.2. Flow — Xem chứng chỉ

```txt
/my-certificates
→ Người dùng chọn chức năng: Xem chứng chỉ
→ Frontend hiển thị form/danh sách tương ứng
→ Người dùng nhập dữ liệu hoặc chọn thao tác
→ Bấm xác nhận
→ Frontend gọi API backend
...
### 15.3. Flow — Xác minh chứng chỉ

```txt
/my-certificates
→ Người dùng chọn chức năng: Xác minh chứng chỉ
→ Frontend hiển thị form/danh sách tương ứng
→ Người dùng nhập dữ liệu hoặc chọn thao tác
→ Bấm xác nhận
→ Frontend gọi API backend
...
| TC06 | Kiểm tra chức năng: Kiểm tra điều kiện nhận chứng chỉ | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
| TC07 | Kiểm tra chức năng: Sinh mã chứng chỉ | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
| TC08 | Kiểm tra chức năng: Xem danh sách chứng chỉ | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
| TC09 | Kiểm tra chức năng: Tải file chứng chỉ | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
| TC10 | Kiểm tra chức năng: Xác minh chứng chỉ bằng mã | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
| TC11 | Kiểm tra chức năng: Hiển thị thông tin khóa học và người học trên chứng chỉ | Hệ thống xử lý đúng nghiệp vụ và cập nhật dữ liệu chính xác |
...
Use-case **Quản lý chứng chỉ** là một phần quan trọng trong hệ thống LMS. Tài liệu này mô tả chi tiết từ phạm vi nghiệp vụ, database, API, luồng dữ liệu, sequence, activity flow đến prototype UI và test cases. Khi triển khai, nên bám sát phân quyền và ràng buộc dữ liệu để đảm bảo hệ thống ổn định, dễ mở rộng và dễ bảo trì.
